...
{
   int	iNroCampo;
   char szDescripcion[64];
} str_descripcion_campo;
...
typedef struct
{
	int iTipoMensaje;
	char szCodProceso[7];
	char szDescripcion[128];
} str_descripcion_tipo_mensaje;
...
public:
   ...
   int ChkValid(int nro);		/* Verifica si es un registro 	   */
                                	/* para ese mensaje.		   */
   int ChkReady();			/* Verifica si estan los registros */
					/* mandatorios			   */
   int GetMsgType();			/* Devuelve el tipo de mensaje	   */
   int GetMsgString(char *msg_string);	/* Devuelve el msj. armado (ISO)   */
   char *GetDescripcionCampo(int nro);
   char *GetDescripcionTipoMensaje();   
};
