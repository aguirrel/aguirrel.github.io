/************************************************************************/
/*									*/
/* X25Daemon.C	    : Proceso que envia las transacciones de credito    */
...
void main(int argc, char *argv[])
{
    ...
    while (1)
        {
        LogAlarm.Put(10, "X25Daemon::main(): Esperando mensaje...\n");

        len=ret=XQueue.GetMsg(0, msg_str, sizeof(msg_str));
        if (ret==NOOK)
            {                       // Hubo error 
            queueErrs++;
            LogAlarm.Put( 0, "X25Daemon::main(): "
        ...
        for (MsgSended=0; !MsgSended; )
            ...
            if (counter<NRO_CHANS )
                {
                LogAlarm.Put(10, "X25Daemon::main(): "
...
int SendX25Msg(X25 unX25, char *msgBuffer, int largoMsg)
{
    ...
    LogAlarm.Put(5, "X25Daemon::SendX25Msg(): "
    ...
    LogAlarm.Put(0, "X25Daemon::SendX25Msg(): "
    ...
    switch (ret)
        ...
        case -1:   // Error en Fork.
            LogAlarm.Put( 1, "X25Daemon::SendX25Msg(): "
    ...
    if (ret==NOOK)
        ...
        time_t horaErr;
        horaErr=time(&horaErr);
        LogAlarm.Put( 5, "X25Daemon::SendX25Msg(): %s",
			asctime(localtime(&horaErr)));
    ...
    if (RtaIsoMsg->ChkField(42)!=TRUE)
         {
        LogAlarm.Put(25, "X25Daemon::SendX25Msg(): "
...
static int getCaFromMsg(char * DaemonMsg,int largoMsg)
{
    ...
    LogAlarm.Put(25, "X25Daemon::getCaFromMsg(): "
    ...
    LogAlarm.Put(5, "X25Daemon::getCaFromMsg(): "
...
void init()
{
    ...
    time_t horaErr;
    horaErr=time(&horaErr);
    LogAlarm.Put( 5, "X25Daemon::init(): %s",
                     asctime(localtime(&horaErr)));

    // Cola de Eventos del Daemon Principal.
    Cfg.GetItem("VisaDaemon", "QueueID", cfgString);
    LogAlarm.Put(10, "X25Daemon::init(): Abriendo cola de eventos...\n");
    ...
    LogAlarm.Put(10, "X25Daemon::init(): "
    ...
    LogAlarm.Put(30, "X25Daemon::init(): Abriendo Controlador de Status..\n");
...
static void shut_down(int sig_id)
{
    ...
    for(counter=0;counter<NRO_CHANS;counter++)
        {
        LogAlarm.Put(0, "X25Daemon::Cerrando canal X.25...\n");
        ret=X25LogiCha[counter]->Close();
        if (ret==NOOK)
            {
            LogAlarm.Put(0,"X25Daemon::shut_down: " 
                           "Error (%d) al Cerrar Canal X.25!!\n", 
                           X25LogiCha[counter]->GetErrno());
            exit(4);
            }
        } 
    LogAlarm.Put(0, "X25Daemon::Borrando PID de la tabla de PIDs...\n");
    ret=PTable.RemPid(getpid());
    if (ret==NOOK)
        {
        LogAlarm.Put(0, "X25Daemon::Error (%d)!!\n", PTable.GetErrno());
        }

    LogAlarm.Put(0, "X25Daemon::Cerrando cola de X.25...\n");
    ret=XQueue.Destroy();
    if (ret==NOOK)
        {
        LogAlarm.Put(0, "X25Daemon::Error (%d)!!\n", XQueue.GetErrno());
    ...
    LogAlarm.Put(0, "X25Daemon::Hecho.\n");
...
for (counter=0; counter<QtyChan; counter++)
    ...
    LogAlarm.Put(10, "X25Daemon::OpenCaChannels(): "
    ...
    if (ret==NOOK)
        {
        LogAlarm.Put(1, "X25Daemon::OpenCaChannels(): "
...
LogAlarm.Put(10, "X25Daemon::CloseCaChannels(): "
...
LogAlarm.Put( 5, "X25Daemon::chkOrdenes(): "
                 "Test para Archivo: [%s]\n",fileCmd); 
...
LogAlarm.Put( 0,"X25Daemon::chkOrdenes(): Hora Actual(): %s", 
...
if ((rc==0) && (sec > Time2Reset)) // Pudo Borrar el archivo ==> Ergo Existia..
    ...
    hora_ult=time(&hora_ult);
    LogAlarm.Put( 0,"X25Daemon::chkOrdenes(): Hora de reseteo de canal(): %s", 
