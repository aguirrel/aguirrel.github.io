...
void pvShutDown(int sig_id=0)
{
    ...
    if (iRet==NOOK)
    {
        LogAlarm.Put(0, "SendRev: Error (%d) al cerrar CACS flag\n", CACS.GetErrno());
    }
...
void pvInit()
{
	...
	Cfg.GetItem("AdminDaemon", "LogSend", szLogSend);
	...
	while (CACS.Open(atol(szCacs))==NOOK)
	{
		...
		{
			LogAlarm.Put(0, "SendRev: Error (%d) al abrir CACS\n", CACS.GetErrno());
			pvShutDown();
		}
	}
	...
	if (iRet==NOOK)
	{
		LogAlarm.Put(0, "SendRev: Error (%d) al actualizar CACS\n", CACS.GetErrno());
	}
...
+-----------------+------------------------------------------------------------+
*/
int piCanalesLevantados(char *nro_ca)
{
	int	 iRet;
	char szComando[256];
	char szCtrlCom[256];

	memset(szCtrlCom, 0, sizeof(szCtrlCom));
	Cfg.GetItem("AdminDaemon", "ControlChannel", szCtrlCom);
	sprintf(szComando, "%s %s 1>/dev/null 2>/dev/null", szCtrlCom, nro_ca);
	iRet = system(szComando);
	iRet = ( (iRet<0) || (iRet>64) ) ? 0 : iRet;
	LogAlarm.Put(0, "SendRev: %s verifica canales. CA:[%s]. Rta:[%02d]\n", szCtrlCom, nro_ca, iRet);
	return (iRet);
}
/*
+-----------------+------------------------------------------------------------+
...
int SendTrx(char *nro_ca, char *cod_tar, char *cod_est, int event_type)
{
	...
	while (iRet == OK)
	{
		...
		LogAlarm.Put(0, "SendRev: Genera evento. Mensaje:[%s]\n", RstEventData );

		/* Envia mensaje al Daemon para que procese */
		iRet = EQueue.SendMsg(event_type, RstEventData, strlen(RstEventData));
		if (iRet==NOOK)
		{
		...
		else
		{
			...
			alarm(60);
			iRet = AQueue.GetMsg(getpid(), RtaData, sizeof(RtaData));
			alarm(0);
			...
			if (strncmp(RtaData, "TOUT", 4)==0)
			{
				...
				if ( piCanalesLevantados(nro_ca) >0 )
				{
